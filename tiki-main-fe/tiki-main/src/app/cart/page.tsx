...
const [cart, setCart] = useState<CartItem[]>([]);
  ...
  useEffect(() => {
    const fetchCart = async () => {
      const response = await fetch('/api/cart');//API*** get carts
  ...
  const handleQuantityChange = async (productId:any, quantity:any) => {
    ...
    // If you have an API for updating quantities, implement the update logic here
      ...
      <div>
        ...
        <div className='w-full flex flex-col bg-white mt-2 pt-5 rounded-md mb-5'>
          {cart.map((product: any) => (
            <ItemCart  onQuantityChange={handleQuantityChange} onRemove={handleRemoveItem} product={product} />
          ))}
        </div>
      </div>
