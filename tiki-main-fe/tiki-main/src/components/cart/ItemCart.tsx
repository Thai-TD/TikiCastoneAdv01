...
export const ItemCart = ({ product, onRemove, onQuantityChange }: PropsType) => {
  const [amount, setAmount] = useState(1);

  useEffect(() => {
    // Update the quantity in the parent component whenever it changes
    onQuantityChange(product.id, amount);
  }, [amount, onQuantityChange, product.id]);
  ...
  return (
    <div className='ml-2 flex mb-5'>
      ...
      <div className='text-gray-500 w-[10%] pr-5 flex justify-end items-center'>
        <TrashIcon className='size-5'  onClick={() => onRemove(product.id)} />
      </div>
    </div>
  );
};
